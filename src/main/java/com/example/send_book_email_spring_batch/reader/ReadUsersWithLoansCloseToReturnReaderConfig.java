package com.example.send_book_email_spring_batch.reader;

import com.example.send_book_email_spring_batch.domain.Book;
import com.example.send_book_email_spring_batch.domain.User;
...
public class ReadUsersWithLoansCloseToReturnReaderConfig {
    ...
    int numDaysToNotifyRetrun = GenerateBookReturnDate.numDaysToReturnBook - 1;
    ...
    public ItemReader<UserBookLoan> readUsersWithLoansCloseToReturnReader(@Qualifier("appDS") DataSource dataSource) {
        return new JdbcCursorItemReaderBuilder<UserBookLoan>()
                ...
                .sql("SELECT " +
                        ...
                        "WHERE DATE_ADD(loan_date, INTERVAL " + numDaysToNotifyRetrun + " DAY) = DATE(NOW());")
                .rowMapper(rowMapper())
                .build();
    }
    ...
    private RowMapper<UserBookLoan> rowMapper() {
        return new RowMapper<UserBookLoan>() {
            ...
            public UserBookLoan mapRow(ResultSet rs, int rowNum) throws SQLException {
                User user = new User(rs.getInt("user_id"), rs.getString("user_name"), rs.getNString("user_email"));
                Book book = new Book();
